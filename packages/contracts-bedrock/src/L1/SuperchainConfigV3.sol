...
/// @title SuperchainConfigV3
/// @notice Guardian-rotation upgrade of SuperchainConfig: identical storage layout and logic to
///         SuperchainConfig v2.4.x, but built with ReinitializableBase(3) so that
///         `initialize(address _guardian)` can run once more after an upgrade. Guardian rotation
///         therefore follows the standard ProxyAdmin upgrade flow:
///         ProxyAdmin.upgradeAndCall(proxy, this, abi.encodeCall(SuperchainConfigV3.initialize, (newGuardian)))
/// @dev Storage layout is byte-for-byte identical to SuperchainConfig (same bases in the same
///      order, no added state variables). Guardian rotation does not clear existing pause states.
contract SuperchainConfigV3 is ProxyAdminOwnedBase, Initializable, ReinitializableBase, ISemver {
    ...
    /// @notice Constructs the SuperchainConfigV3 contract.
